...
**KNOWN LIVE-VALIDATION GAP**: the running Android worker was started before T021 and is still serving pre-T021 code. The auth smoke against the live emulator MUST be re-run after worker restart (and rebuilding the API container per CLAUDE.md "Critical: Task Queue Must Match Worker") to confirm the planner-prompt overhaul and plan-graph guard don't regress the auth flow on real device traffic.
