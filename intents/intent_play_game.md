...
3. **Action** from `playbook.actions_json`: slots/Slingo → `spin`; blackjack → `hit`/`stand`/`double`/`split`; roulette → `place_bet` only if window ≥`min_bet_window_ms`. **First-launch (actions_json empty)**: derive the action via `FindElementWithFallback` against the kind's HIGH-risk element names (slots → `spin_button`, blackjack → `hit_button`/`stand_button`, roulette → grid cells). Auto-recorder persists the resolved selector to `screen_elements`; next round reuses it.
...
- **Advance** with `FindElementWithFallback` against prominent text candidates: `Continue`, `Next`, `Pick`, `Reveal`, `Collect`. **NEVER tap** `Buy`, `Deposit`, `Add Funds`, `Extra Spins`.
